...
std::vector<std::string> dic(max_word);
std::unordered_map<char,std::pair<int,int>> t_search; // {alpha,pos_st,pos_ed}
hash_word *hash_table[NHASH]={nullptr};
...
int main(){
    int pos_d = 1,pos_pre = 0;
    char t_st = 'a';
    std::ifstream in("dictionary.txt",std::ios::in);
    std::string temp_s;
    ...
    while(std::cin>>temp_s>>func){
        switch(func){
            case 1 :    
                ...
                break;
            case 2 :
                binary_search(1,pos_d-1,temp_s);
                break;
            case 3 :
                t_st = temp_s.front();
                binary_search(t_search[t_st].first,t_search[t_st].second,temp_s);
                break;
            case 4 :
                hash_search(temp_s);
                break;
