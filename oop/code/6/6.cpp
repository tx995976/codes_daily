...
class Rectangle : public Point {
    ...
    public:
        ...
        double get_distance(Point input){
            return sqrt(pow(this->point_for_rectangle.getx()-input.getx(),2)+pow(this->point_for_rectangle.gety()-input.gety(),2));
        }
};
...
class Circle : public Point {
    ...
    public:
        ...
        void cout_judge_by_point(Point input);    
        double get_distance(Point input){
            return sqrt(pow(this->Point_for_Circle.getx()-input.getx(),2)+pow(this->Point_for_Circle.gety()-input.gety(),2));
        }
};
...
void Rectangle::cout_judge_by_point(Point input){
    if(input.getx() < this->getx()||input.gety() > input.gety())
        cout<<"在矩形外"<<endl;
